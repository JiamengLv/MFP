...
parser.add_argument('--outf', default='checkpoints/cycle/', help='folder to output images and model checkpoints')
...
def test(epoch):
    ...
    Ab = np.zeros(shape=(b, z, h, w))
    for i in range(b):
        Ab[i] = LoadSaveFits.norm2(AB[i], z)
    AB = torch.from_numpy(Ab).float()

    b, z, h, w = BA.shape
    BA = BA.data.cpu().numpy()
    Ba = np.zeros(shape=(b, z, h, w))
    for i in range(b):
        Ba[i] = LoadSaveFits.norm2(BA[i], z)
    BA = torch.from_numpy(Ba).float()

    ABA = G_BA(AB)
    BAB = G_AB(BA)

    # identity loss
    l_idt = (criterionMSE(AB, label_B) + criterionMSE(BA, label_A))
    ...
    LoadSaveFits.save_fit_cpu(I.data, 'I_%03d_' % epoch, './out_picture/cycle/out_image_test/')
    LoadSaveFits.save_fit_cpu(Abs_Br.data, 'Abs_Br_%03d_' % epoch, './out_picture/cycle/out_image_test/')
    LoadSaveFits.save_fit_cpu(Bt.data, 'Bt_%03d_' % epoch, './out_picture/cycle/out_image_test/')
...
for epoch in range(opt.epoch,opt.n_epoch+1):
    ...
    ###########   Logging   ############
    if(epoch % opt.log_step):
        print('[%d/%d] Loss_errG: %.4f  Loss_idt1: %.4f  Loss_idt2: %.4f loss_recon: %.4f'
                  % (epoch, opt.epoch,
                       errG.data,criterionMSE(AB,label_B).data,criterionMSE(BA,label_A).data,errMSE))
    lossdata1.append('[%d/%d] Loss_errG: %.4f  Loss_idt1: %.4f  Loss_idt2: %.4f loss_recon: %.4f'
                  % (epoch, opt.epoch,
                       errG.data,criterionMSE(AB,label_B).data,criterionMSE(BA,label_A).data,errMSE))
    ...
    if(epoch % opt.log_step == 0):
        ...
        LoadSaveFits.save_fit_cpu(I.data,'I_%03d_'%epoch,'./out_picture/cycle/out_image_train/')
        LoadSaveFits.save_fit_cpu(Abs_Br.data,'Abs_Br_%03d_'%epoch,'./out_picture/cycle/out_image_train/')
        LoadSaveFits.save_fit_cpu(Bt.data, 'Bt_%03d_' % epoch, './out_picture/cycle/out_image_train/')
