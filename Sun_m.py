...
parser.add_argument('--outf', default='checkpoints/Sun_m/', help='folder to output images and model checkpoints')
...
def test(epoch):
    ...
    LoadSaveFits.save_fit_cpu(I.data,'I_%03d_'%epoch,'./out_picture/Sun_m/out_image_test/')
    LoadSaveFits.save_fit_cpu(Abs_Br.data,'Abs_Br_%03d_'%epoch,'./out_picture/Sun_m/out_image_test/')
    LoadSaveFits.save_fit_cpu(Bt.data, 'Bt_%03d_' % epoch, './out_picture/Sun_m/out_image_test/')
...
for epoch in range(opt.epoch,opt.n_epoch+1):
    ...
    if(epoch % opt.log_step == 0):
        ...
        LoadSaveFits.save_fit_cpu(I.data,'I_%03d_'%epoch,'./out_picture/Sun_m/out_image_train/')
        LoadSaveFits.save_fit_cpu(Abs_Br.data,'Abs_Br_%03d_'%epoch,'./out_picture/Sun_m/out_image_train/')
        LoadSaveFits.save_fit_cpu(Bt.data, 'Bt_%03d_' % epoch, './out_picture/Sun_m/out_image_train/')
